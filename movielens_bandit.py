...
MovieLens 10M Dataset, which is released by GroupLens at 1/2009. Please first pre-process
...
#     experiment_bandit = ['LinThompSamp']
#     regret = {}
#     col = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'w']
#     i = 0
...
def main():
    ...
    seq_error = policy_evaluation(policy, bandit, streaming_batch_small, user_feature, reward_list,
                                      actions, action_context)
    regret[bandit] = regret_calculation(seq_error)
    plt.plot(range(len(streaming_batch_small)), regret[bandit], c='r', ls='-', label=bandit)
    plt.xlabel('time')
    plt.ylabel('regret')
    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
    axes = plt.gca()
