...
from bandits import thomp_cab, thomp_multi, thomp_one
...
def policy_generation(bandit, dim, t, numUsers):
    ...
    if bandit == 'ThompCAB':
        policy = thomp_cab.ThompCAB(numUsers, d=dim, gamma=0.05, delta=0.1, 
                                    R=0.02, epsilon=1/np.log(t/numUsers), random_state=4)

    elif bandit == 'Thompson_single':
        pass
...
def main():
    ...
    X, Y, users, model, users_feat = artificial_data_generator(T=10000, d=5, K=20, classes=1, numUsers=5, model=model)
    ...
    # run algorithms
    bandits = ['ThompsonOne', 'random', 'ThompMulti', 'ThompCAB']
    for i, bandit in enumerate(bandits):
        ...
        plt.plot(range(time), cum_regret[bandit], c=col[i], ls='-', label=str(bandit))
        plt.xlabel('time')
        plt.ylabel('regret')
        axes = plt.gca()
